...
for i in range(len(events_list_m)):
    ...
    for j in range(len(years)):
        mean_year_event_m = event_m.loc[(event_m['Date_Y'] == years[j]), 'Mark_seconds'].mean()
        means_m.append(mean_year_event_m)
    means_w = []
    for j in range(len(years)):
        mean_year_event_w = event_w.loc[(event_w['Date_Y'] == years[j]), 'Mark_seconds'].mean()
    ...
    model_interaction = smf.ols(formula='y ~ time + olympic + gender + time : gender + gender : olympic',
                                data=data_matrix_df)
...
basic_params_df.to_csv("/Users/tassjames/Desktop/Olympic_data/Howard_interaction/basic_params_track.csv")
basic_pvals_df.to_csv("/Users/tassjames/Desktop/Olympic_data/Howard_interaction/basic_pvals_track.csv")

interaction_params_df = pd.DataFrame(interaction_params_list)
interaction_pvals_df = pd.DataFrame(interaction_pvalues_list)
interaction_params_df.to_csv("/Users/tassjames/Desktop/Olympic_data/Howard_interaction/interaction_params_track.csv")
interaction_pvals_df.to_csv("/Users/tassjames/Desktop/Olympic_data/Howard_interaction/interaction_pvals_track.csv")